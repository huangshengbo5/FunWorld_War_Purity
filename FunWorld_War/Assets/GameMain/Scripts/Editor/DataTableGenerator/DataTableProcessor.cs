...
namespace StarForce.Editor.DataTableTools
{
    public sealed partial class DataTableProcessor
    {
        ...
        public string GetDefaultValue(int rawColumn)
        {
            ...
            return m_DefaultValueRow != null ? m_DefaultValueRow[rawColumn] : " ";
        }
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(memoryStream, Encoding.UTF8))
                {
                    for (int rawColumn = 0; rawColumn < RawColumnCount; rawColumn++)
                    {
                        ...
                        {
                            var defaultValue = GetDefaultValue(rawColumn);
                            if (m_DataProcessor[rawColumn].IsId )
                            {
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    m_DataProcessor[rawColumn].WriteToStream(this, binaryWriter, defaultValue);
                                }
                                catch
                                {
